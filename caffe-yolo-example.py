...
def show_results(img, results, out_path):
...
def detect(model_filename, weight_filename, img_filename, mode):
    ...
    if mode == 'darknet':
        ...
        output_path = img_filename[0:-4] + '_output.jpg'
        img_cv = cv2.cvtColor(cv2.imread(img_filename), cv2.COLOR_RGB2BGR)
        results = get_candidate_objects(out['result'][0], img.shape, mode)
        show_results(img_cv, results, output_path)
        cv2.waitKey()
